...
int main() {
    std::cout << "\n=== DBX4 Storage Engine - Production Build ===" << std::endl;
    std::cout << "Page Size: " << dbx4::PAGE_SIZE << " bytes" << std::endl;
    std::cout << "Max Slots/Page: " << dbx4::MAX_SLOTS_PER_PAGE << std::endl;
    std::cout << std::endl;

    int passed = 0, failed = 0;

    // Test 1-100: CRC32C & Data Integrity (100 tests)
    for (int i = 0; i < 100; i++) {
        uint8_t data[256];
        memset(data, i % 256, sizeof(data));
        uint32_t crc = dbx4::CRC32C::calculate(data, sizeof(data));
        if (dbx4::CRC32C::verify(data, sizeof(data), crc)) {
            passed++;
        } else {
            failed++;
        }
    }
    std::cout << "[CRC32C Tests] " << passed << "/100 passed" << std::endl;

    // Test 101-200: Page Operations (100 tests)
    passed = failed = 0;
    for (int i = 0; i < 100; i++) {
        dbx4::Page page(i);
        uint8_t data[64];
        memset(data, i % 256, sizeof(data));
        uint16_t slot;
        if (page.insert_row(data, sizeof(data), 1, slot) && page.get_row_count() == 1) {
            passed++;
        } else {
            failed++;
        }
    }
    std::cout << "[Page Insert Tests] " << passed << "/100 passed" << std::endl;

    // Test 201-350: Page Serialization (150 tests)
    passed = failed = 0;
    for (int i = 0; i < 150; i++) {
        dbx4::Page page(i);
        uint8_t data[128];
        memset(data, i % 256, sizeof(data));
        uint16_t slot;
        page.insert_row(data, sizeof(data), 1, slot);
        auto serialized = page.serialize();
        dbx4::Page page2(i);
        if (page2.deserialize(serialized) && page2.get_row_count() == 1) {
            passed++;
        } else {
            failed++;
        }
    }
    std::cout << "[Page Serialization Tests] " << passed << "/150 passed" << std::endl;

    // Test 351-450: MVCC Transactions (100 tests)
    passed = failed = 0;
    for (int i = 0; i < 100; i++) {
        dbx4::MVCCManager mvcc;
        uint64_t txn = mvcc.begin_transaction();
        if (mvcc.commit_transaction(txn) && mvcc.get_active_txn_count() == 0) {
            passed++;
        } else {
            failed++;
        }
    }
    std::cout << "[MVCC Transaction Tests] " << passed << "/100 passed" << std::endl;

    // Test 451-550: Buffer Pool Operations (100 tests)
    passed = failed = 0;
    dbx4::BufferPool pool(64, "./dbx4_test");
    for (int i = 0; i < 100; i++) {
        auto page = pool.get_page(i);
        if (page && page->get_page_num() == i) {
            passed++;
        } else {
            failed++;
        }
    }
    std::cout << "[Buffer Pool Tests] " << passed << "/100 passed" << std::endl;
    std::cout << "[Buffer Pool Hit Ratio] " << (pool.get_hit_ratio() * 100) << "%" << std::endl;

    // Test 551-700: Storage Engine Ops (150 tests)
    passed = failed = 0;
    dbx4::StorageEngine engine(128, "./dbx4_engine_test");
    uint32_t table = engine.create_table("test_table");
    for (int i = 0; i < 150; i++) {
        uint64_t txn = engine.begin_transaction();
        uint8_t data[32];
        memset(data, i % 256, sizeof(data));
        if (engine.insert_row(table, data, sizeof(data), txn) && engine.commit_transaction(txn)) {
            passed++;
        } else {
            failed++;
        }
    }
    std::cout << "[Storage Engine Tests] " << passed << "/150 passed" << std::endl;

    // Performance benchmark (10,000 inserts)
    {
        auto start = std::chrono::high_resolution_clock::now();
        dbx4::StorageEngine perf_engine(256, "./dbx4_perf");
        uint32_t perf_table = perf_engine.create_table("perf_table");
        uint64_t txn = perf_engine.begin_transaction();
        
        for (int i = 0; i < 10000; i++) {
            uint8_t data[64];
            memset(data, i % 256, sizeof(data));
            perf_engine.insert_row(perf_table, data, sizeof(data), txn);
        }
        perf_engine.commit_transaction(txn);
        
        auto end = std::chrono::high_resolution_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
        double throughput = (10000.0 * 1000.0) / duration.count();
        
        std::cout << "[Performance Benchmark] 10,000 inserts in " << duration.count() << "ms" << std::endl;
        std::cout << "[Throughput] " << static_cast<int>(throughput) << " rows/sec" << std::endl;
    }

    // Concurrent operations
    {
        dbx4::StorageEngine concurrent_engine(256, "./dbx4_concurrent");
        uint32_t concurrent_table = concurrent_engine.create_table("concurrent_table");
        
        std::vector<std::thread> threads;
        for (int i = 0; i < 10; i++) {
            threads.emplace_back([&concurrent_engine, concurrent_table, i]() {
                for (int j = 0; j < 100; j++) {
                    uint64_t txn = concurrent_engine.begin_transaction();
                    uint8_t data[32];
                    memset(data, (i * 100 + j) % 256, sizeof(data));
                    concurrent_engine.insert_row(concurrent_table, data, sizeof(data), txn);
                    concurrent_engine.commit_transaction(txn);
                }
            });
        }
        ...
        std::cout << "[Concurrent Operations] 10 threads x 100 inserts completed" << std::endl;
    }
