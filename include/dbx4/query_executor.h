...
public:
    ...
    std::string to_string() const { switch (type) { case NULL_T: return "NULL"; case INT: return std::to_string(int_val); case DOUBLE: { std::string s = std::to_string(double_val); s.erase(s.find_last_not_of('0') + 1, std::string::npos); if (s.back() == '.') s.pop_back(); return s; } case TEXT: return text_val; } return ""; }
};
struct Table {
    std::string name;
...
private:
    ...
    static std::vector<std::string> split_quoted(const std::string& str, char delim);
    static std::string extract_table_name(const std::string& sql);
    static void validate_trailing(const std::string& sql, size_t pos);
